...
use crate::{WorldSettings, LevelSet, WorldSet, obstacle::{new_obstacle, ObstacleMaterials, RegionRef, HitObstacleEvent}, scoring_region::new_scoring_region, GameState, ResetEvent};
...
fn setup_level_settings(world_settings: Res<WorldSettings>,
    ...
    level_settings.max_object_width = level_settings.obstacle_width.max(level_settings.scoring_gap_width);
    level_settings.start_offset = world_settings.bounds.max.x + 100.0;
}

fn update_timer(time: Res<Time>,
...
fn spawn_obstacles(mut commands: Commands,
	...
	let vel = Velocity { linvel: level_settings.vel, ..default() };

	let lvl = &level_settings;

	let gap_center = lvl.center_y_range[0] +  fastrand::f32() * (lvl.center_y_range[1] - lvl.center_y_range[0]);
	let gap_height = lvl.gap_height_range[0] +  fastrand::f32() * (lvl.gap_height_range[1] - lvl.gap_height_range[0]);
	...
	let region = commands.spawn(new_scoring_region(
	    1,
	    Vec2::new(level_settings.start_offset + level_settings.obstacle_width - scoring_gap_width/2.0, gap_center),
	    Vec2::new(scoring_gap_width, scoring_gap_height))).
	    insert((RemoveWhenLeft, RemoveOnReset, vel.clone())).id();

	commands.spawn(new_obstacle(true, top_height, &mut meshes, &play_world, &level_settings, &obs_mat))
	    .insert((Name::new("top_obstacle"),
		     RegionRef { region },
		     RemoveWhenLeft,
		     RemoveOnReset,
		     vel.clone()));
	commands.spawn(new_obstacle(false, bottom_height, &mut meshes, &play_world, &level_settings, &obs_mat))
	    .insert((Name::new("bottom_obstacle"),
		     RegionRef { region },
		     RemoveWhenLeft,
		     RemoveOnReset,
		     vel.clone()));
...
fn remove_invisible_objects(
    ...
    query: Query<(Entity, &GlobalTransform), (With<RemoveWhenLeft>, With<Collider>)>,
    play_world: Res<WorldSettings>,
    level_settings: Res<LevelSettings>) {
    for (ent, global) in query.iter() {
...
fn reset_on_collision(mut resets: EventWriter<ResetEvent>,
...
impl Plugin for LevelPlugin {
	...
	app.insert_resource(LevelSettings::default())
	    .insert_resource(LevelTimer { timer: Timer::from_seconds(2.0, TimerMode::Repeating) })
	    .add_systems(Startup, setup_level_settings.in_set(LevelSet).after(WorldSet))
	    .add_systems(PreUpdate, update_timer)
	    .add_systems(Update, (remove_invisible_objects,
				  spawn_obstacles,
				  reset_on_collision)
